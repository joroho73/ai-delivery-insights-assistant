...
def build_html_report(
    ...
    <p>Generated by <a href="https://www.linkedin.com/in/johnroberthoward/" target="_blank">John Howard</a>'s <em> AI Project Delivery Insights Assistant</em>
    <br/>On<em> {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}</em></p>
    ...
    <div style="font-size: 0.8em; color: #666; margin-top: 40px;">
        <p>Report generated by AI Project Delivery Insights Assistant</p>
        <p>Copyright (c) {datetime.now().strftime("%Y")} <a href="https://www.linkedin.com/in/johnroberthoward/" target="_blank">John Howard</a>. All rights reserved.</p>
...
### How to use this App
Upload a `.csv` file of project delivery information to visualise performance metrics, and generate an AI-assisted portfolio analysis.
"""
)
...
- `Project` - the name or identifier of the project
- `Status` - the current delivery status (e.g. On Track, At Risk, Off Track)
- `Budget` - the allocated budget for the project (numbers only, no currency symbols)
- `ActualCost` - the actual cost of the project (numbers only, no currency symbols)
- `DelayDays` - the number of days the project is delayed (numbers only, negative if ahead of schedule)
- `RiskLevel` - the level of risk associated with the project (e.g. Low, Medium, High)
"""
)

st.write("If you need a template .csv file, you can download one here.")

# Set a session state variable to track whether the user has chosen to use demo data, 
# so we can show the data preview and charts immediately without needing to wait for a file upload.

if "use_demo_data" not in st.session_state:
    st.session_state.use_demo_data = False

with open("data/project_delivery_template.csv", "rb") as template_file:
    st.download_button(
        label="Download CSV Template",
        data=template_file,
        file_name="project_delivery_template.csv",
        mime="text/csv"
    )

st.markdown("### Upload Project Delivery .csv file")
...
# if user uploads a real file, disable demo mode
if uploaded_file is not None:
    st.session_state.use_demo_data = False

st.markdown("Or use a demo dataset to try the app immediately.")
...
if uploaded_file is not None or st.session_state.use_demo_data:
    ...
    if st.session_state.use_demo_data:
        df = pd.read_csv("data/project_data_30_messy_projects.csv")
    else:
